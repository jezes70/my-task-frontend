...
const Token = () => {
  ...
  useEffect(() => {
    const fun = async () => {
      try {
        setHoled(false);
        const url = `http://localhost:${PORT}/api/v1/ve`;
      ...
      {!loading ? (
        ...
        <>
          <BodyToken>
            <Modar>
              <img
                src={statusCode === 200 ? activeIcon : inact}
                alt=""
                width={50}
              />
              ...
              <p>{mainMessage}</p>
              <p>
                Welcome to Customer Support! Our platform is designed to provide
                you with an exceptional user experience
              </p>
              <ButtonEmail type="button" onClick={login}>
                Go to Login
              </ButtonEmail>
            </Modar>
          </BodyToken>
